...
from app.forms import LoginForm, RegistrationForm, EditProfileForm, \
    ...
    AddressForm, EditForm, DeleteForm
...
def edit_profile():
    ...
    if form.validate_on_submit():
        ...
        current_user.linkedinlink = form.linkedinlink.data
        db.session.commit()
        ...
        form.linkedinlink.data = current_user.linkedinlink
...
def edit(address_id):
    ...
    return render_template('edit.html', title=_('Edit'),
                           form=form, address= address)


@app.route('/delete/<address_id>', methods=['POST'])
@login_required
def delete(address_id):
    address = Address.query.filter_by(id=address_id).first_or_404()
    form = DeleteForm(current_user.username)
    if form.validate_on_submit():
        g.db.execute('delete from address where id = address_id', [request.form['address']])
        db.session.commit()
        flash(_('Your changes have been saved.'))
        return redirect(url_for('edit', address_id=address.id))
        return render_template('edit.html', title=_('Edit Address'), form=form, address=address)
...
def edit_address():
    ...
    page = request.args.get('page', 1, type=int)
    addresses = current_user.addresses.order_by(Address.timestamp.desc()).paginate(page, 25, False)
    ...
    return render_template('edit_address.html', title=_('Edit Address'),
                           form=form, addresses=addresses.items, next_url=next_url,
                           prev_url=prev_url)
...
def delete(address_id):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(_('Your changes have been saved.'))
        return render_template('edit_address.html', title=_('Edit Address'), form=form, address=address)
